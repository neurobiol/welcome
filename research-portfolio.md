# Research Portfolio

This page provides a public overview of my current and recent research activities across quantum biology, neurobiology, memory and plasticity, artificial life, transcriptomics, quantum sensing, and AI/cognition.

My thesis work is focused on quantum-like adaptive-plasticity models for context-sensitive associative memory. The items below are related research, training, collaboration, dissemination, and service activities. They are listed here as a living research portfolio rather than as thesis deliverables.

---

## Current thesis focus

**Quantum-like adaptive plasticity for context-sensitive associative memory**

This work develops and tests adaptive-plasticity models of memory using matched quantum-like, real-valued no-phase, and Markov-rate model families. The broader goal is to understand how recall strength, order sensitivity, context dependence, interference/recovery, and stage structure can be preserved across memory dynamics.

---

## Manuscripts, preprints, and collaborative research

### Quantum biology and quantum neurobiology

- **Quantum in Biology, Quantum for Biology, and Biology for Quantum: Mapping the Evidence and the Road Ahead**  
  Role: Third author  
  Status: Preprint

- **Quantum Effects in Neurobiology**  
  Role: Co-author with Prof. T.J.A. Craddock  
  Status: Submitted to collaborators in the United Kingdom  
  Date: To add

- **Quantum Sensing for Biology: From Physical Sensitivity to Biological Inference**  
  Role: Second author  
  Status: Manuscript ready for submission to *PRX Life*  
  Date: To add

- **Ultraweak Photon Emission: A Multiscale Framework for Biological Sources, Measurement, Signal Processing, and Future Biomedical Use**  
  Role: Collaborator  
  Status: In progress  
  Collaboration: Laurier University

---

### Artificial life and theoretical biology

- **How Topology Shapes Synchrony in Directed Signed Memristive FitzHugh--Nagumo Networks**  
  Role: Second author  
  Status: Accepted

- **Can Life Exist “Inside” an Atom?**  
  Role: First author  
  Status: Accepted

- **Quantum-Like Artificial Life with Memory and Plasticity**  
  Role: Proposal author  
  Status: Proposal submitted for *Artificial Life: Biology and Computation*  
  Note: Course audited

---

### Quantum foundations and biological modeling

- **Gödel--Turing Noncomputability in Finite Quantum Measurement: A Conditional Framework for Microtubule-Like Realization**  
  Role: Author / collaborator  
  Status: Manuscript ready for submission  
  Collaboration: University of British Columbia

- **From Biological Networks to Quantum-Like Graphs: Directionality, Dynamics, and Probabilistic Structure**  
  Role: Author  
  Status: Manuscript completed for BIOL 680

---

### Transcriptomics and bioinformatics

- **A Delayed Pulsed-Field Transcriptomic Program Emerges from Six-Condition RNA-seq Analysis**  
  Role: Collaborator  
  Status: Project in collaboration with Laurier University

- **Transcriptomics project completed for BIOL 614**  
  Role: Author  
  Status: Completed and presented  
  Presentation date: To add

---

### AI, cognition, and psychology

- **Smarter in Which Sense? A Diagnostic Framework for AI--Human Intelligence Claims**  
  Role: Collaborator  
  Status: In progress  
  Collaboration: János Szentágothai Neuroscience Doctoral School, Semmelweis University, Budapest, Hungary

- **Remembering Fallibility: AI, Memory, and Intellectual Humility**  
  Role: Proposal author  
  Status: Proposal submitted for PSYCH 650  
  Date: To add

---

## Funding and proposal activities

- **PSSO proposal funding application**  
  Role: Applicant  
  Status: Submitted  
  Date: To add

- **Bergel proposal funding application**  
  Role: Applicant  
  Status: Shortlisted for two interviews among 15 out of approximately 650  
  Date: To add

---

## Mentoring and service

- **Remote mentoring of an undergraduate student at Toronto Metropolitan University, Department of Chemistry and Biology**  
  Role: Mentor  
  Topic: Academic article on Parkinson's disease

- **Reviewer for URNCST Journal**  
  Role: Reviewer  
  Status: Ongoing service activity

---

## Presentations, posters, and awards

- **PHYS10, University of Waterloo**  
  Type: Poster presentation  
  Date: To add

- **PATH, University of Waterloo**  
  Type: Poster / flash talk  
  Award: Best Flash Talk Award  
  Date: To add

- **The Quantum Biology Forum, Silver Spring, United States**  
  Type: Poster presentation  
  Award: Graduate Dissemination Award from the University of Waterloo  
  Date: To add

- **QUEBS, Hungary**  
  Type: Poster presentation  
  Date: To add

- **QIP 2026, Sweden**  
  Type: Talk  
  Award: Graduate Dissemination Award from the University of Waterloo

---

## Contact

For questions or collaboration inquiries, please contact me by institutional email.

Last updated: 2026.

